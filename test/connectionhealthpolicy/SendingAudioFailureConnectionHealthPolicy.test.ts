...
import SendingAudioFailureConnectionHealthPolicy
  from "../../src/connectionhealthpolicy/SendingAudioFailureConnectionHealthPolicy";
...
describe('SendingAudioFailureConnectionHealthPolicy', () => {
  ...
  beforeEach(() => {
    ...
    sendingAudioFailureHealthPolicy = new SendingAudioFailureConnectionHealthPolicy(logger, configuration, data);
  });
  ...
  it('can return maximum health when connection start is recent and audio packets are not being sent', () => {
    data.setConsecutiveStatsWithNoAudioPacketsSent(configuration.sendingAudioFailureSamplesToConsider);
    data.setConnectionStartTime();
    clock.tick(10);
    sendingAudioFailureHealthPolicy.update(data);
    expect(sendingAudioFailureHealthPolicy.health()).to.equal(sendingAudioFailureHealthPolicy.maximumHealth());
  });
  ...
  it('can return minimum health when connection start is not recent and audio packets are not being sent', () => {
    ...
    sendingAudioFailureHealthPolicy.update(data);
    expect(sendingAudioFailureHealthPolicy.health()).to.equal(sendingAudioFailureHealthPolicy.minimumHealth());
  });
  ...
  it('can simulate warning not raised due to recent warning and raised after cool down time elapses', () => {
    data.setConnectionStartTime();
    data.setConsecutiveStatsWithNoAudioPacketsSent(configuration.sendingAudioFailureSamplesToConsider); //First unhealthy
    clock.tick(configuration.sendingAudioFailureInitialWaitTimeMs);
    sendingAudioFailureHealthPolicy.update(data);
    expect(sendingAudioFailureHealthPolicy.health()).to.equal(sendingAudioFailureHealthPolicy.minimumHealth());
    ...
    data.setConsecutiveStatsWithNoAudioPacketsSent(configuration.sendingAudioFailureSamplesToConsider); //Again unhealthy immediately but warned recently so reports healthy
    sendingAudioFailureHealthPolicy.update(data);
    expect(sendingAudioFailureHealthPolicy.health()).to.equal(sendingAudioFailureHealthPolicy.maximumHealth());
