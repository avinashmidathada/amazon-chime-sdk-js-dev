  ...
  async sendingAudioCheck(stepInfo, expectedEvent, waitTimeMs) {
    ...
    try {
      ...
      logs.forEach(entry => {
        ...
        if (matchedItems) {
          //When running on Saucelabs the logs are weirdly escaped with just a single \ instead of \\ so JSON.parse is
          //throwing an error, so we escape all instances of \" with "
